...
export default async function Usage() {
  ...
  const stripeClientNonNull = stripeClient!;
  const customer = await stripeClientNonNull.customers.list({
  ...
  const meteredPricesWithTiers = await Promise.all(
    subscription?.items.data
      ...
      .map(async (item) => {
        ...
        const priceDetails = await stripeClientNonNull.prices.retrieve(item.price.id, {
          expand: ['tiers']
        });

        // Get current usage
        const usage = item.price.recurring?.meter && await stripeClientNonNull.billing.meters.listEventSummaries(
